...
import { setCurrentPatient, setDocuments, setSelectedDocument } from '../../store/slices/documentsSlice';
...
const DocumentBrowsingPage = () => {
  ...
  const { documents, selectedDocumentId, activeFilters } = useAppSelector((state) => state.documents);
  ...
  const getDocumentIcon = (type: Document['type']) => {
    switch (type) {
      case 'Pathology':
        return <ExperimentOutlined style={{ fontSize: 24, color: '#722ed1' }} />;
      case 'Clinical Notes':
        return <FileTextOutlined style={{ fontSize: 24, color: '#1890ff' }} />;
      case 'Radiology':
        return <FilePdfOutlined style={{ fontSize: 24, color: '#52c41a' }} />;
      case 'Lab Results':
        return <ExperimentOutlined style={{ fontSize: 24, color: '#faad14' }} />;
      case 'Treatment':
        return <MedicineBoxOutlined style={{ fontSize: 24, color: '#ff4d4f' }} />;
      default:
  ...
  const handlePrevious = () => {
    if (canGoPrevious) {
      navigate(`/documents/${selectedPatientIds[currentPatientIndex - 1]}`);
    }
  };
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Layout>
        <Sider width={250} theme="light" style={{ background: '#fafafa', padding: 16 }}>
          <Card size="small" style={{ marginBottom: 16 }}>
            <Space direction="vertical" size="small">
              ...
              <Text type="secondary">{currentPatient?.primaryDiagnosis}</Text>
            </Space>
          </Card>

          <Title level={5}>Selected Patients</Title>
          <List
            size="small"
            dataSource={selectedPatientIds.map((id) => patients.find((p) => p.id === id))}
            renderItem={(patient) => (
              <List.Item
                style={{
                  background: patient?.id === patientId ? '#e6f7ff' : 'transparent',
                  cursor: 'pointer',
                  padding: 8,
                  borderRadius: 4,
                }}
              ...
              >
                <Text strong={patient?.id === patientId}>{patient?.name}</Text>
              </List.Item>
            )}
          />
        </Sider>
        ...
        <Content style={{ padding: 24, background: '#fff' }}>
          <Space direction="vertical" style={{ width: '100%' }} size="large">
            <Card title="Document Timeline" size="small">
              <Timeline
                mode="left"
            ...
            <Card>
              <Space wrap style={{ marginBottom: 16 }}>
                {[
                  ...
                  <Badge count={docTypeCount(type)} key={type}>
                    <Button
                      type={selectedFilter === type ? 'primary' : 'default'}
                ...
                renderItem={(doc) => (
                  <List.Item>
                    <Card
                      hoverable
                      onClick={() => {
                        dispatch(setSelectedDocument(doc.id));
                        setDrawerOpen(true);
                      }}
                    >
                      <Space direction="vertical" style={{ width: '100%' }}>
                        ...
                        <Tag color={getFormatColor(doc.format)}>{doc.format}</Tag>
                      </Space>
                    </Card>
                  </List.Item>
                )}
            ...
            <Card style={{ position: 'fixed', bottom: 0, left: 250, right: 0, zIndex: 100 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Button type="primary" size="large" onClick={() => navigate('/ontology-selection')}>
      ...
      >
        {selectedDocumentId && (() => {
          const doc = patientDocs.find((d) => d.id === selectedDocumentId);
          ...
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="large">
              <div>
                ...
                <Space size="large">
                  <Text>
                    <Text strong>Date:</Text> {doc?.date}
                  </Text>
                  <Text>
                    <Text strong>Provider:</Text> {doc?.provider}
                  </Text>
                  <Tag color={getFormatColor(doc?.format || 'Text')}>{doc?.format}</Tag>
                  <Tag>{doc?.type}</Tag>
                </Space>
              </div>
